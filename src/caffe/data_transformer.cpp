...
DataTransformer<Dtype>::DataTransformer(const TransformationParameter& param,
  ...
  if (param_.channel_factor().size() > 0) {
    for (int i = 0; i < param_.channel_factor().size(); ++i) {
      channel_factors_.push_back(param_.channel_factor(i));
    }
  }  
}
...
void DataTransformer<Dtype>::TransformImgAndSeg(
	...
	cv::Mat cv_img = cv_img_seg[0];
	cv::Mat cv_seg = cv_img_seg[1];
	...
	const int seg_channels = cv_seg.channels();
	...
	const bool has_mean_file = param_.has_mean_file();
	const bool has_mean_values = mean_values_.size() > 0;
	const bool contrast_adjust = param_.contrast_adjustment();
	...
	if (phase_ == TRAIN) {
		...
		int flipping_mode = 2;
		if (param_.mirror())
			flipping_mode = (Rand(2)) + 1; // 1, 2
		if (param_.flip())
			flipping_mode = (Rand(4)) - 1; // -1, 0, 1, 2
		if (flipping_mode != 2) {
				cv::flip(cv_img, cv_img, flipping_mode);
				cv::flip(cv_seg, cv_seg, flipping_mode);
		}
		
		// smooth filtering
		if (smooth_filtering) {
			int smooth_param1 = 3;
			smooth_param1 += 2 * (Rand(1));
			int smooth_type = Rand(5);
			switch (smooth_type){
			case 0:
		...
		if (contrast_adjust) {
			...
			if (apply_contrast) {
				...
				Dtype min_alpha = 0.8, max_alpha = 1.2;
		...
		// rotation
		if (rotation) {
			int rotation_angle_interval = param_.rotation_angle_interval();

			int interval = 360 / rotation_angle_interval;
			int apply_rotation = Rand(interval);

			double rotation_degree = apply_rotation * rotation_angle_interval;
			double sinVal = fabs(sin(rotation_degree / 180 * CV_PI));
			double cosVal = fabs(cos(rotation_degree / 180 * CV_PI));
			int rotate_width = int(img_height * sinVal + img_width * cosVal);
			int rotate_height = int(img_height * cosVal + img_width * sinVal);

			int maxSize = std::max(img_width, img_height)*1.414;
			int dx = (maxSize - img_width) / 2;
			int dy = (maxSize - img_height) / 2;

			cv::Mat cv_img_border;
			cv::Mat cv_seg_border;

			if (has_mean_values){
				cv::copyMakeBorder(cv_img, cv_img_border, dy, dy, dx, dx, cv::BORDER_CONSTANT,
			...
			cv::copyMakeBorder(cv_seg, cv_seg_border, dy, dy, dx, dx, cv::BORDER_CONSTANT,
				cv::Scalar(ignore_label));

			cv::Point2f center(cv_img_border.cols / 2., cv_img_border.rows / 2.);
			cv::Mat map_matrix = getRotationMatrix2D(center, rotation_degree, 1.0);

			warpAffine(cv_img_border, cv_img_border, map_matrix, cv_img_border.size());
			warpAffine(cv_seg_border, cv_seg_border, map_matrix, cv_seg_border.size());

			//crop boder
			dx = (maxSize - rotate_width) / 2;
			dy = (maxSize - rotate_height) / 2;
		...
		//channel scaling
		if (channel_adjust) {
			CHECK(channel_factors_.size() == 1 || channel_factors_.size() == img_channels)
				<< "Specify either 1 mean_value or as many as channels: " << img_channels;
			if (img_channels > 1 && channel_factors_.size() == 1) {
				// replicate the mean_value for simplicity
				for (int c = 1; c < img_channels; ++c) {
					channel_factors_.push_back(channel_factors_[0]);
				}
			}
			for (int h = 0; h < img_height; ++h) {
				uchar* data_ptr = cv_img.ptr<uchar>(h);
				int data_index = 0;
				for (int w = 0; w < img_width; ++w) {
					for (int c = 0; c < img_channels; ++c) {
						data_ptr[data_index++] *= channel_factors_[c];
					}
				}
			}
		}
		...
		//size scaling
		Dtype min_scaling = param_.min_scale_factor();
		Dtype max_scaling = param_.max_scale_factor();
		if (min_scaling != 1 || max_scaling != 1) {
			CHECK_GT(max_scaling, min_scaling)
				<< "max_scaling_factor should greater than min_scaling_factor";
			int scale_rand = Rand(img_height);
			float size_scale = min_scaling + (scale_rand / img_height)*(max_scaling - min_scaling);
			cv::Size new_iamge_size(img_width*size_scale, img_height*size_scale);
			cv::resize(cv_img, cv_img, new_iamge_size, 0, 0,
				cv::INTER_LINEAR);
			cv::resize(cv_seg, cv_seg, new_iamge_size, 0, 0,
				cv::INTER_NEAREST);
			// after scaling, need pad mean pixel
			int pad_height = std::max(img_height - cv_img.rows, 0);
			int pad_width = std::max(img_width - cv_img.cols, 0);
			if (pad_height > 0 || pad_width > 0) {
				...
				if (has_mean_values){
					...
					cv::copyMakeBorder(cv_img, cv_img, 0, pad_height,
						0, pad_width, cv::BORDER_CONSTANT, cvAvg(&cv_img));
				}
	...
	for (int h = 0; h < data_height; ++h) {
		data_ptr = cv_img.ptr<double>(h);
		label_ptr = cv_seg.ptr<uchar>(h);
